...
class TestRotateEmbeddings:
    ...
    def test_half_precision(self, device):
        ...
        atol_half = torch.maximum(embeddings_half_error, rope_half_error) * 10
        atol_bf16 = torch.maximum(embeddings_bf16_error, rope_bf16_error) * 10

        # Verify errors didn't explode during the operation
        assert torch.allclose(
            rotated_full, rotated_half.float(), atol=float(atol_half.item()), rtol=1e-2
        )
        assert torch.allclose(
            rotated_full, rotated_bf16.float(), atol=float(atol_bf16.item()), rtol=5e-2
        )
...
class TestRotateEmbeddingsBackward:
    ...
    def test_half_precision(self, device):
        ...
        atol_half = (
            max(
                grad_emb_rot_half_error.item(),
                embeddings_half_error.item(),
                rope_half_error.item(),
            )
            * 10
        )
        atol_bf16 = (
            max(
                grad_emb_rot_bf16_error.item(),
                embeddings_bf16_error.item(),
                rope_bf16_error.item(),
            )
            * 10
        )
